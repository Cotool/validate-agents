---
description: Wire up the cotool/validate-agents GitHub Action in the current repo.
---

Set up the `cotool/validate-agents` GitHub Action in this repository. Follow these steps:
...
1. **Find the agent directory.** Look for a directory containing Cotool agent YAML files
   ...
   `agents/`, or a path named in an existing `.github/workflows/` file. If you can't find one,
   ask the user where their agent files live before continuing.
